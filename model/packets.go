...
// PacketIDs
const (
	PacketIDMotion              = uint8(iota) // PacketMotionData
	PacketIDSession                           // PacketSessionData
	PacketIDLapData                           // PacketLapData
	PacketIDEvent                             // PacketEventData
	PacketIDParticipants                      // PacketParticipantsData
	PacketIDCarSetups                         // PacketCarSetupData
...
type PacketSessionData struct {
	...
	SafetyCarStatus     uint8           // 0 = no safety car, 1 = full safety car
...
/*
 * Convert C++ union to Go is a bit complicated
 * Also, I don't care about this kind of data, so just ignore it
 */
...
//         float32    lapTime;    // Lap time is in seconds
//     } FastestLap;
...
//         float32 speed;      // Top speed achieved in kilometres per hour
//     } SpeedTrap;
// };
...
type CarStatusData struct {
	...
	// Added in Beta3:
	DrsActivationDistance uint16 // 0 = DRS not available, non-zero - DRS will be available
	// in [X] metres
